...
def test_customer_defaults():
    ...
    """
    c = Customer(id=2, username="jane", email="jane@test.com", password_hash="h")
...
def test_customer_coordinate_validation():
    ...
    with pytest.raises(ValueError, match="Latitude must be between -90 and 90"):
        Customer(id=3, username="x", email="x@x.com", password_hash="h", latitude=100.0)
    
    # Test Longitude
    with pytest.raises(ValueError, match="Longitude must be between -180 and 180"):
        Customer(id=3, username="x", email="x@x.com", password_hash="h", longitude=-200.0)
...
def test_customer_with_empty_address_logic():
    ...
    """
    c = Customer(id=1, username="u", email="e@e.com", password_hash="h", address="")
    assert c.address == ""
    assert c.latitude == 0.0
